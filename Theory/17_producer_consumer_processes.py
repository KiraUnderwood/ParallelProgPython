...
'''
demo of gil with threading module. with cpu intence opetation in consumer the q will be constantly overflown
gil allows only 1 thread to exec at the time so with spou intence operation the problem is not solved - > need multiprocessing 
import queue from the multiprocessing package
pass serviceng line as an input arg to teh producer and consumer functions

but if consumer is io boud the threading will work just fine
'''

serving_line = mp.Queue(5)

def cpu_work(work_units):  # what if the consumer is cpu intensive opetations, no io bound? lets simulate cpu intensive operation which keeps cpu busy
    x = 0
