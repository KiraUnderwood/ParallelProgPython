...
'''
IO boud problem. limiting factor is internet speed
more CPU won't make any difference so threading module will suite just fine - > ThreadPoolExecutor
...
def par_download_images(image_numbers):
    total_bytes = 0
    with concurrent.futures.ThreadPoolExecutor() as pool:   # divide and conqure approach
        futures = [pool.submit(_download_image, num) for num in image_numbers]  # wait for the result of the functions
