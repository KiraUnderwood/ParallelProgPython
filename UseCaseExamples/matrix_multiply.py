...
def par_matrix_multiply(A, B):
    ...
    # if the output C will be small enough, simply use the sequential version since it is ususally faster
    ...
    chunk_size = math.ceil(num_rows_A/num_workers)  # divide into equal chunks
    C_1D = mp.RawArray('d', num_rows_A * num_cols_B) # flat version of matrix C, shared array to be used amond several processes, does not include synchronization. do not need sync here as the task is very parallel
    workers = []
...
if __name__ == '__main__':
    NUM_EVAL_RUNS = 1
    A = [[random.random() for i in range(1000)] for j in range(1000)]  # effective only on lage numbers, 200 x 200 - > not effective
