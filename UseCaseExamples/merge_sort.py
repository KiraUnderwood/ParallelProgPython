...
def par_mergesort(array, *args):  # we are limited by the available processors
    if not args: # first call
        shared_array = mp.RawArray('i', array)  # all sub-processes will be interacted, initialized with the initial array elements
        par_mergesort(shared_array, 0, len(array)-1, 0) # third arg to track the depth the resursion
        array[:] = shared_array # insert result into original array
        return array
    else:
        left, right, depth = args
        if (depth >= math.log(mp.cpu_count(), 2)):
            seq_mergesort(array, left, right)  # if we reached the sufficient depth based on avail prpcessors n teh system it calls original alg
        elif (left < right):
            mid = (left + right) // 2
            left_proc = mp.Process(target=par_mergesort, args=(array, left, mid, depth+1))  #
            left_proc.start()
            par_mergesort(array, mid+1, right, depth+1)  # use current process to recursivelly sort the right half
            left_proc.join()
